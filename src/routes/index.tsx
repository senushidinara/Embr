        ...
        <div className="flex items-start justify-between gap-3">
          <div className="pointer-events-auto glass rounded-2xl px-3.5 py-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="mono text-[10px] text-muted-foreground hidden sm:inline">
                day-by-day · week 1–4
              </span>
            </div>
          ...
          <div className="pointer-events-auto glass rounded-2xl px-3.5 py-2.5 min-w-[160px] md:min-w-[220px]">
            ...
            <div className="mt-0.5 text-sm md:text-base font-semibold leading-tight">
              {day.title}
            </div>
            <dl className="mt-1.5 grid grid-cols-2 gap-x-2 gap-y-0.5 text-[10px] md:text-[11px]">
              <dt className="text-muted-foreground">Size</dt>
              <dd className="mono">{day.size}</dd>
              <dt className="text-muted-foreground">Cells</dt>
              <dd className="mono truncate">{day.cellCount}</dd>
            </dl>
          </div>
        </div>
      </header>
        ...
        <div className="glass rounded-2xl p-2.5 md:p-3 space-y-3">
          <ControlBlock title="View">
            ...
            <div>
              <div className="flex items-center justify-between text-[11px] text-muted-foreground mb-1">
                <span>Explode</span>
                <span className="mono">{Math.round(explode * 100)}%</span>
              </div>
              <input
                type="range"
                min={0}
          ...
          <ControlBlock title="Slice">
            ...
            <div>
              <div className="flex items-center justify-between text-[11px] text-muted-foreground mb-1">
                <span>Depth</span>
                <span className="mono">{sliceDepth.toFixed(2)}</span>
              </div>
              <input
                type="range"
                min={-1}
      ...
      <div className="absolute right-2 md:right-4 top-28 md:top-32 z-10 w-[220px] md:w-[320px] max-h-[calc(100vh-260px)]">
        <div className="glass rounded-2xl overflow-hidden flex flex-col max-h-[calc(100vh-260px)]">
          ...
          >
            ...
            <span className={`mono text-[10px] transition ${panelOpen ? "" : "rotate-180"}`}>
              ▾
            </span>
          </button>
                  ...
                  <div className="p-3.5 text-[12px] leading-relaxed">
                      ...
                      (day.signalingPathways.length ? (
                        <ul className="space-y-2.5">
                          {day.signalingPathways.map((s, i) => (
                            ...
                            >
                              <div className="mono text-[10px] text-primary uppercase tracking-wider">
                                {s.pathway}
                              </div>
                        ...
                        <p className="text-muted-foreground">
                          No key signaling pathways tracked for this day.
                        </p>
                      ))}
                    {tab === "events" && (
                      <ul className="space-y-1.5">
                        {day.keyEvents.map((e, i) => (
                          <li key={i} className="flex gap-2">
                            <span className="mono text-[10px] text-primary/70 mt-0.5">
                    ...
                    {tab === "molecular" &&
                      (day.molecular.length ? (
                        ...
                        <p className="text-muted-foreground">
                          No key molecular events tracked for this day.
                        </p>
                      ))}
          ...
          <div className="mt-2.5 flex gap-1 overflow-x-auto no-scrollbar pb-0.5">
            {DAYS.map((d, i) => (
              ...
              >
                ...
                <div className="text-[10px] font-semibold leading-tight truncate max-w-[80px]">
                  {d.title}
                </div>
              </button>
            ))}
...
function ToggleRow({
  ...
  return (
    ...
    >
      <span>{label}</span>
      <span className={`mono text-[10px] ${active ? "opacity-90" : "opacity-60"}`}>
        {active ? "ON" : "OFF"}
      </span>
    </button>
  );
